...
# def test_find_all_funds():
#     with patch('config.db.conn.bd_btg.fund.find') as mock_find:
#         mock_find.return_value = [mock_fund_data()]
...
#     with patch('config.db.conn.bd_btg.fund.insert_one') as mock_insert:
#         mock_insert.return_value.inserted_id = "1"
#         with patch('config.db.conn.bd_btg.fund.find_one') as mock_find_one:
...
#     with patch('config.db.conn.bd_btg.fund.find_one_and_update') as mock_update:
#         with patch('config.db.conn.bd_btg.fund.find_one') as mock_find_one:
...
def test_delete_fund():
    fund_id = "1"
    with patch('config.db.conn.bd_btg.fund.find_one_and_delete') as mock_delete:
